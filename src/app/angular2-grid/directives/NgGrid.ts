...
export class NgGrid implements OnInit, DoCheck, OnDestroy {
	...
	private _updateSize(col?: number, row?: number): void {
		...
		this._renderer.setElementStyle(this._ngEl.nativeElement, 'width', '70%');//(maxCol * (this.colWidth + this.marginLeft + this.marginRight))+'px');
		// Remove this when necessary
		this._renderer.setElementStyle(this._ngEl.nativeElement, 'float', 'left');
    ...
    public initPlaceholder() {
        let factory = this.componentFactoryResolver.resolveComponentFactory(NgGridPlaceholder)
        let componentRef = this._containerRef.createComponent(factory)
        this._placeholderRef = componentRef;
        this._placeholderRef.instance.registerGrid(this);
        this._placeholderRef.instance.setSize({x: 0,y: 0});
    }
