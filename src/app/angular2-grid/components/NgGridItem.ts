...
import { Component, ViewChild, Directive, ElementRef, Renderer, EventEmitter, Host, HostListener, ViewEncapsulation, Type, ComponentRef, KeyValueDiffer, KeyValueDiffers, OnInit, OnDestroy, DoCheck, ViewContainerRef, Output, Input } from '@angular/core';
...
@Component({
    selector: '[ngGridItem]',
    template: `
        <div class="handle">Widget {{ngGridItem.id}} <div class="box-header-btns pull-right">
			<a title="Remove widget">
				<i class="glyphicon glyphicon-trash"></i>
			</a>
		</div></div>
    `,
	styleUrls: ['NgGridItem.css']
})
...
export class NgGridItem implements OnInit, OnDestroy {
    ...
    private mouseMove$:ReplaySubject<number> = new ReplaySubject();
    private mouseDown$:ReplaySubject<number> = new ReplaySubject();
    private resize$:ReplaySubject<number> = new ReplaySubject();

	//	[ng-grid-item] handler
	@Input()
    private ngGridItem:NgGridItem;
	...
	//	Constructor
	constructor(private _differs: KeyValueDiffers, private _ngEl: ElementRef, private _renderer: Renderer, public _ngGrid: NgGrid, public containerRef: ViewContainerRef) { }
    ...
    private injectComponent():void {
        console.log('in inject component');
    }
